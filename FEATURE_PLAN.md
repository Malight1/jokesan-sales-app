...
**Date:** 12 September 2026. **Status updated:** 18 September 2026 — see `HANDOVER.md` for the fuller picture.
**Scope:** the seven items from the competitor review (see `COMPETE_ROADMAP.md` for the why).
**Starts from:** migrations 0001–0036, **all live in production** — every phase in this document has shipped its database side. The `assistant` Edge Function itself (Phase 7b) still isn't deployed, and now targets Groq instead of Anthropic (the user didn't want to commit to paid API usage before the feature proved itself) — see `HANDOVER.md`'s "Where things actually stand" and "Setting up the assistant." Work commits directly to `main` (no feature branches — see `HANDOVER.md`'s hard constraints).
...
**Every phase in this document has now been attempted, and every migration through 0036 is live.** The one thing still not live is the `assistant` Edge Function itself (Phase 7b) — it needs deploying and a `GROQ_API_KEY` secret (not `ANTHROPIC_API_KEY` — the provider changed after `0036` shipped) before it actually answers anything (see `HANDOVER.md`). What's genuinely left, across the whole plan, is narrower than a phase: 5b/5c (dedicated virtual accounts, a bank feed), 7a's own finished-goods "produce" suggestion, 7c's provider adapter, and the assistant's own small deferred items — see `HANDOVER.md`'s "Known gaps deferred so far" for the complete, current list. Cross-check specifics (column names, function signatures) against what actually exists before assuming anything below this line is still accurate — it's the plan as originally written, not a live status report.
...
- Built: the `assistant` Edge Function, calling a model with the exact tool set this plan names (`dashboard_summary`, `stock_levels`, `reorder_suggestions`, `batch_trace`, and all three `report_*` functions), each one called "with the user's own login" exactly as this plan specifies — a Supabase client built from the asking user's own JWT, so a role's normal restrictions apply automatically, with no special-casing needed in the assistant itself. Business plan only, with a monthly question limit per business (`assistant_usage`), matching this plan's "Limits and rollout" section.
- **Originally built against Anthropic (Claude Haiku 4.5), switched to Groq (free tier, OpenAI-compatible) once shipped** — the user didn't want to commit to paying for API usage before knowing whether customers would actually use the feature. The provider lives entirely in `supabase/functions/assistant/index.ts`; nothing in the database or frontend needed to change. Worth remembering Groq's free tier is one shared rate-limit bucket across every StockFlow tenant, not per business.
- Not built: this plan's own "cheap model for everyday questions, larger model for analysis" split (one model is used throughout — routing well between two models needs a real classifier this session didn't build) and the "later" MCP server for asking from outside the app.
